...
async function load(me) {
  async function xhr() {
    ...
    headers.append('X-Pinterest-App-Type-Detailed', '8');
    // set charset
    ...
    if ((global.ctrl || {}).clientId) {
      headers.append('X-Client-ID', global.ctrl.clientId);
    }
    // are we signed in?
    if (me.auth && me.xRequestForgeryToken) {
      debug('Setting X-Request-Forgery-Token to ' + me.xRequestForgeryToken);
      headers.append('X-Request-Forgery-Token', me.xRequestForgeryToken);
    }
    // create request options
    let reqOptions = {
      // method = get except if specified
      method: me.method || 'GET',
      headers: headers,
      responseType: me.responseType || 'json',
    };

    // add formData to request if sent
    if (me.formData) reqOptions.body = me.formData;
    // create request obj
    let req = new Request(me.url, reqOptions);
    debug(`Fecthing: ${me.url}`);
    debug(`headers ${String(headers)}`);
    const res = await fetch(req);

    if (res.status === 200) {
      const out = { response: await res.json() };
      ...
      return { response: { status: 'fail', error: 'API Error' } };
    }
  }
...
function disableFeatures(me) {
  ...
  if (me.button) {
    browser.action
      ...
      .then(() => debug('Icon set'))
      .catch(() => debug('Error while setting icon'));
  }
}
...
function injectLogic(me) {
  ...
  */
  ...
  browser.scripting.executeScript({
    target: { tabId: me.tabId },
    files: [config.inject.logic],
  });
}
...
function refreshContextMenus(me) {
  ...
  if (me.data.nopin) {
    ...
    browser.action
      .setIcon({ path: 'img/icon_toolbar.png' })
      .then(() => debug('Toolbar icon set'))
      .catch(() => debug('Error while setting the Toolbar Icon'));
    browser.contextMenus.removeAll();
    // save
    try {
      ...
      if (!global.hideSearch) {
        ...
        browser.contextMenus.create(
          ...
          () => {
            debug('Adding search click event listener');
...
function injectPinmarklet(me) {
  ...
  browser.scripting.executeScript({
    target: { tabId: me.tabId },
    files: [config.inject.pinmarklet],
  });
}
...
function populateSearch(me) {
  ...
  browser.tabs.captureVisibleTab((uri) => {
    ...
    sendOut({
      to: 'save',
      act: 'renderSearch',
      data: { ...me.data, method: 'r', searchMe: uri },
    });
  });
}
...
function housekeep() {
  ...
  config.browserTest.filter((it) => {
    if (!userAgentFound && navigator.userAgent.match(it.r)) {
