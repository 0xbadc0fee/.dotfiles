  ...
  */
  ...
  function getImageData(me) {
    return new Promise((resolve) => {
      ...
      // start working here
      debug('Trying to collect image data');
      ...
      if (config.pattern.alwaysConvertToData.some((pattern) => pattern.test(me.url))) {
  ...
  function save(me) {
    ...
    if (me.name === 'click') {
      ...
      if (!global.hazPinningNow) {
        ...
        const pinsToSaveConverted = [];

        for (let pin of pinsToSave) {
          getImageData({ url: pin.media || '' }).then((imageConverted) => {
            pin = { ...pin, imageConverted: imageConverted };
            pinsToSaveConverted.push(pin);
  ...
  */
  ...
  function getImageData$1(me) {
    ...
    return new Promise((resolve) => {
      ...
      // is this image from a source we need to fetch and convert to a data URI before sourcing?
      if (pattern.alwaysConvertToData.some((pattern) => pattern.test(me.url))) {
        // this fetch will come from disk cache so it will be very fast
        fetch(me.url)
          .then(function (response) {
            return response.blob();
  ...
  */
  ...
  function runSearch(me) {
    if (me.u) {
      ...
      if (query.data.img || query.data.u) {
        if (query.data.img && !query.data.u) {
          ...
          getImageData$1({
            ...
            query.data.img = imgData.dataURI;
            send(query);
          });
        }
      } else {
  ...
  function newBoardWin(me) {
    ...
    addFromTemplate({
      ...
      template: {
        [data.id]: {
          addClass: 'item',
          boardName: data.name.toLowerCase(),
          cover: {},
          info: {
            text: data.name,
          },
    ...
    */
    ...
    if (me.data) {
      ...
      me.data.thumb.filter((thumb) => {
        // is this image from a source we need to fetch and convert to a data URI before sourcing?
        if (config.pattern.alwaysConvertToData.some((pattern) => pattern.test(thumb.media))) {
